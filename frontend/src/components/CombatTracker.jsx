      ...
      <div style={{
        ...
        {activeConc && (
          <div style={{
            ...
            <span>🔮</span>
            <span>Concentrating on <strong>{activeConc}</strong> — casting another concentration spell will end this.</span>
          </div>
        )}
        ...
        {/* Spell slot display — Warlock gets a distinct pact slot banner */}
        {isWarlock ? (
          <div style={{
            display: 'flex',
            alignItems: 'center',
            ...
            <div>
              <div style={{ fontSize: '13px', fontWeight: 'bold', color: pactSlotsLeft > 0 ? '#CE93D8' : '#555' }}>
                Pact Magic Slots — Level {pactSlotInfo.slotLevel}
              </div>
              <div style={{ fontSize: '11px', color: '#888' }}>
                {pactSlotsLeft} / {pactSlotInfo.slotCount} remaining
                {pactSlotsLeft === 0 && ' — Short rest to recover!'}
              </div>
            </div>
            {/* Slot pips */}
        ...
        {selectedSpell && (
          <div style={{
            ...
            {/* Upcast selector — hidden for Warlocks (no slot level choice) */}
            ...
            {/* Warlock auto-cast info — shows pact slot level, no choice needed */}
            ...
            {!selectedTarget && selectedSpell.damage && (
              <div style={{ color: '#ff9800', fontSize: '12px', marginBottom: '8px' }}>
                ⚠️ Select a target enemy above before casting this spell
              </div>
            )}
...
export default function CombatTracker({ combat, onUpdate, onEnd, party = [], updateParty }) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  //  SHORT REST — Warlocks recover all Pact Magic slots
  // ─────────────────────────────────────────────────────────────────────────
  const handleShortRest = () => {
    ...
    const updatedParty = party.map(p => {
      ...
      addLog(`🔮 ${p.name} takes a short rest and recovers ${pact.slotCount} Pact Magic slot${pact.slotCount > 1 ? 's' : ''} (level ${pact.slotLevel})!`);
      anyRecovered = true;
      return recovered;
    });

    if (!anyRecovered) {
      addLog('⛺ Short rest taken. (Only Warlocks recover spell slots on a short rest.)');
    }
  ...
  const handleConcentrationCheck = (character, damageTaken) => {
    ...
    if (check.passed) {
      return {
        character,
        logEntry: `🔮 ${character.name} passes concentration check (rolled ${check.roll} vs DC ${check.dc}) — still concentrating on ${character.concentrationSpell}.`
      };
    } else {
      const cleared = clearConcentration(character, 'failed concentration check');
      return {
        character: cleared,
        logEntry: `💔 ${character.name} FAILS concentration check (rolled ${check.roll} vs DC ${check.dc}) — ${character.concentrationSpell} ends!`
      };
    }
  };
  ...
  const handleAttack = (attackIndex = 0) => {
    ...
    const logEntry = result.attackRoll.isCritical
      ...
      ? `❌ FUMBLE! ${result.attacker} misses ${result.target}!`
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', paddingBottom: '15px', borderBottom: '2px solid #2a2a2a' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {/* Short rest — always visible; especially important for Warlocks */}
          <button
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '15px', overflowX: 'auto', paddingBottom: '10px' }}>
        {combat.combatants.map((combatant, index) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div style={{ position: 'absolute', top: '-18px', left: '50%', transform: 'translateX(-50%)', fontSize: '18px' }}>⬇️</div>
              )}
              <div style={{ fontSize: '11px', color: combatant.isEnemy ? '#ff6b6b' : '#4CAF50', marginBottom: '4px' }}>
                {combatant.isEnemy ? '👹' : '🛡️'} Init: {combatant.initiative}
              </div>
              ...
              {combatant.isDead && (
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '36px', backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: '8px' }}>💀</div>
              )}
            </div>
          );
        })}
      ...
      <div style={{ backgroundColor: '#1a1a1a', padding: '15px', borderRadius: '8px', border: '2px solid #333' }}>
        <h3 style={{ margin: '0 0 12px 0', color: '#ffd700', fontSize: '16px' }}>
          ...
          {isPlayerTurn && isSpellcaster && (
            <span style={{ marginLeft: '10px', fontSize: '12px', color: '#CE93D8' }}>✨ Spellcaster</span>
          )}
              ...
              {currentCombatant.attacks?.map((attack, index) => (
                ...
                >
                  ...
                  <span style={{ display: 'block', fontSize: '10px', color: selectedTarget ? '#81C784' : '#444', fontWeight: 'normal' }}>
                    +{attack.bonus} to hit • {attack.damage}
                  </span>
                </button>
              ))}
